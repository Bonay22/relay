...
- `doc/GO-KNOWLEDGE-MAP.md` задаёт темы, зависимости, доказательства понимания
  и глобальные статусы;
...
- текущий `doc/NN-name.md` хранит выбранные ID и доказательства конкретного
  этапа.

Каждый этап проходит одинаково:

1. Ментор выбирает из глобальной карты один небольшой кластер ID и проверяет,
   что все его предпосылки уже имеют статус `understood`.
2. Ментор объясняет один кластер с нуля: назначение, термины, синтаксис, варианты
   и минимальные примеры, не связанные с Relay, если так механизм виден лучше.
3. Перед каждым изменением Relay ментор показывает продуктовую проблему, роль
   файла и каждого нового объявления, а также текущий поток данных. Если код
   пока является заготовкой, отдельно называется его будущий потребитель и то,
   чего этот шаг ещё не делает при запуске.
4. Ученик предсказывает результат, объясняет правило своими словами или
   применяет его в маленьком упражнении. До этого производственное задание не
   выдаётся.
5. Ученик реализует один ограниченный шаг Relay и сообщает, что готов к ревью.
6. Ментор проверяет корректность, идиоматичность, обработку ошибок, тесты и
   читаемость. Замечания делятся на обязательные и рекомендательные.
7. Ментор задаёт несколько вопросов по использованным концепциям. Это не
   экзамен: вопросы нужны, чтобы обнаружить непонятые места.
8. Доказательства записываются в документ этапа, а статусы синхронизируются в
   глобальной карте.
9. После исправлений запускаются проверки этапа.
10. Если результат понятен и критерии выполнены, создаётся коммит.
11. Следующая ветка создаётся от принятого состояния текущего этапа.
...
2. выбранные ID глобальной карты, их статусы и доказательства понимания;
3. исходное состояние и необходимые знания;
4. понятное описание цели и ожидаемого поведения;
5. объяснение новых концепций Go и причин их применения;
6. небольшие примеры кода, не содержащие целиком готового решения;
7. пошаговое задание для самостоятельной реализации;
8. критерии приёмки и команды проверки;
9. принятые по ходу реализации решения и причины выбора;
10. вопросы ученика и важные пояснения ментора;
11. результаты code review: обязательные замечания, рекомендации и исправления;
12. контрольные вопросы и краткие выводы ученика;
13. итог этапа, результаты проверок и согласованное сообщение коммита.

ID в таблице понятий каждого существующего и последующего этапа оформляются
относительными ссылками на автоматические якоря заголовков соответствующих тем
`GO-KNOWLEDGE-MAP.md`. Каждый заголовок третьего уровня содержит только ID,
поэтому из документа этапа можно сразу перейти к полному описанию темы, её
критерию понимания и глобальному статусу без HTML в исходном тексте карты.
В сообщениях ментора любой упомянутый ID также оформляется ссылкой на тот же
ряд карты, включая объяснения, статусы, вопросы, ревью и задания. Поскольку
локальные ссылки Codex из чата не открывают HTML-якоря, в сообщениях используется
абсолютный путь файла с текущим номером строки; относительные `#`-якоря остаются
форматом документов этапов.
...
- проектирование границ пакетов и минимального экспортированного API;
...
Первоначальный roadmap принят в `main` коммитом `cfee446`, а обучение идёт в
ветке `stage/00-bootstrap`. Go инициализирован как модуль, первые сведения об
инструментах подтверждены учеником. Текущий учебный блок и доказательства
описаны в [`00-bootstrap.md`](00-bootstrap.md), глобальный прогресс — в
[`GO-KNOWLEDGE-MAP.md`](GO-KNOWLEDGE-MAP.md).

Следующий учебный блок выбирается по зависимостям глобальной карты. Коммит этапа
создаётся только после реализации, ревью, проверок и явного подтверждения
ученика.
