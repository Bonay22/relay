...
3. Ученик предсказывает результат, объясняет правило своими словами или
   применяет его в маленьком упражнении. До этого производственное задание не
   выдаётся.
4. Ученик реализует один ограниченный шаг Relay и сообщает, что готов к ревью.
5. Ментор проверяет корректность, идиоматичность, обработку ошибок, тесты и
   читаемость. Замечания делятся на обязательные и рекомендательные.
6. Ментор задаёт несколько вопросов по использованным концепциям. Это не
   экзамен: вопросы нужны, чтобы обнаружить непонятые места.
7. Доказательства записываются в документ этапа, а статусы синхронизируются в
   глобальной карте.
8. После исправлений запускаются проверки этапа.
9. Если результат понятен и критерии выполнены, создаётся коммит.
10. Следующая ветка создаётся от принятого состояния текущего этапа.
